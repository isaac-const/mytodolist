...
export default function Register() {
  ...
  const handleRegister = async () => {
    ...
    try {
      const res = await axios.post('/auth/register', { name: name.trim(), email, password });
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput style={styles.input} placeholder="Confirme a Senha" placeholderTextColor="#9E78CF" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry />
      <TouchableOpacity style={styles.button} onPress={handleRegister}><Text style={styles.buttonText}>Cadastrar</Text></TouchableOpacity>
      <TouchableOpacity onPress={() => router.push('/login')}><Text style={styles.linkText}>Já tem conta? Entre</Text></TouchableOpacity>
    </View>
  );
}
